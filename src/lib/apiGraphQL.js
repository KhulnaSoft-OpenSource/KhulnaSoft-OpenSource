...
const operationsDoc = `
  ...
  query FetchGoal($number: Int!) {
    gitHub {
      viewer {
        repository(name: "khulnasoft-opensource-goals") {
          issue(number: $number) {
            id
            body
            title
  ...
  query FetchMemberStatusQuery() {
    gitHub {
      viewer {
        organization(login: "khulnasoft-opensource") {
          viewerIsAMember
        }
      }
    }
  ...
  query FetchRepoCountQuery() {
    gitHub {
      search(query: "khulnasoft-opensource-goals", type: REPOSITORY) {
        repositoryCount
      }
    }
  }
    ...
    gitHub {
      cloneTemplateRepository(
        input: {
          ...
          visibility: PUBLIC
          ownerId: $ownerId
          name: "khulnasoft-opensource-goals"
        }
      ) {
        repository {
          id
